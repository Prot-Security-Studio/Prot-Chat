...
def register():
    if request.method == 'POST':
        ...
        face_recording_method = request.form['face_recording_method']

        if face_recording_method == 'upload':
            face_images_paths = []
            for i in range(1, 11):
                face_image_key = f'face_image_{i}'
                if face_image_key in request.files:
                    uploaded_image = request.files[face_image_key]
                    if uploaded_image and uploaded_image.filename.endswith(('.jpg', '.png')):
                        face_image_path = os.path.join('uploads', uploaded_image.filename)
                        uploaded_image.save(face_image_path)
                        face_images_paths.append(face_image_path)
            if len(face_images_paths) == 0:
                return "请至少上传一张人脸图片。"
            else:
                # 保存人脸图像数组为.npy文件
                face_images_array = []
                for path in face_images_paths:
                    ...
                    if face_image is not None:
                        ...
                        if len(faces) > 0:
                            ...
                            face_images_array.append(face_image)
                if face_images_array:
                    np.save(f'face_images/{username}_faces.npy', np.array(face_images_array))
        else:
            face_images_paths = []
        ...
        cursor = conn.cursor()
        try:
            cursor.execute('INSERT INTO users (username, password, face_images_paths) VALUES (?,?,?)',
                          (username, hashed_password, ','.join(face_images_paths)))
            conn.commit()
            return redirect(url_for('login'))
        except sqlite3.IntegrityError:
            return "用户名已存在，请更换用户名重新注册。"
        finally:
            conn.close()
...
def add_friend():
    if 'user_id' in session:
        if request.method == 'POST':
            ...
            current_friend_list = cursor.fetchone()[0].split(',') if cursor.fetchone()[0] else []
